...
idx_parse(char* filepath) {
    ...
    case 2049: // Labels
        ...
        // Result with only one matrix for all labels
        m = malloc(1 * sizeof(Matrix*));

        // One row for all labels. Cols will be used for indexing
        Matrix* labels = matrix_create(1, size, sizeof(uint8_t));

        size_t labels_read =
            fread((labels->data)[0], sizeof(uint8_t), size, file);
    ...
    case 2051: // Images
        ...
        // Result with only one matrix for all labels
        m = malloc(size * sizeof(Matrix*));

        for (uint32_t i = 0; i < size; i++) {
            // Each row will be an image of size width x height
            Matrix* img =
                matrix_create(1, img_width * img_height, sizeof(uint8_t));
            size_t got = fread(img->data[0], img_width * img_height, 1, file);
            assert(got == 1, "Failed to read image %u", i);

            // add image matrix to array
            m[i] = img;
        }
        break;
    default:
